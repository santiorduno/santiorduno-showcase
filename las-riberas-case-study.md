# Case Study: Las Riberas
> Contexto para Claude Code — reemplaza el archivo `src/constants/projects/las-riberas.ts`

---

## Narrativa del case study

**El reto:** Las Riberas es un desarrollo inmobiliario premium en Hermosillo con 10 años en el mercado, múltiples productos (lotes, casas, villas) y una presencia digital que no reflejaba su posicionamiento ni facilitaba la conversión comercial. El proyecto requería arquitectura de información clara, un design system desde cero y un sitio que funcionara como herramienta de ventas real.

**Mi rol:** Lead Designer & Developer — desde research y arquitectura hasta implementación en Webflow.

**Lo distintivo:** Se usaron heatmaps de Attention Insight para validar decisiones de diseño con comportamiento real de usuarios, no con suposiciones.

---

## Archivo TypeScript — reemplazar completo
...
- Reemplazar el archivo completo `src/constants/projects/las-riberas.ts`
- El orden de los `content[]` blocks es intencional — sigue la narrativa: problema → arquitectura → sistema de diseño → validación → resultado
- `image-6.png` es el heatmap del hero con el CTA 3.3% — usar en order 8
- `image-4.png` e `image-5.png` van juntas en gallery (order 10) — heatmap + click map de la misma sección
- `image-2.png` es el sitemap de FigJam — order 4
- `image-3.png` es el design system — order 6
- `image-1.png` es la sección de amenidades del sitio final — order 11
- `hero.png` es la sección Club Deportivo — order 2
- El video `demo.mp4` ya existe en la carpeta, va al final
- Verificar que el thumbnail en `constants/index.tsx` para este proyecto apunte a `/assets/projects/las-riberas/thumbnail.png`
- Si existe traducción en `src/i18n/content/es/projects/las-riberas.ts`, actualizar los campos `heading` y `body` de cada bloque de texto al español

---

## Actualización en `constants/index.tsx`

El objeto del proyecto en el array `projects[]` también necesita actualizar su `description`:
